...
var fire = preload("res://cenas/fogo_ball.tscn")
...
func sinergia():
	dano_sin = 1*fogo*agua*raio*terra*vento*nature
	pass

func atira(body):
	if atirar_on == true :
		var kl = fire.instance()
		self.call_deferred("add_child",kl)
...
func _on_abre_label_button_down():
	get_tree().get_root().get_node("Base/Canvazao/Info_panel/Label").textao(fogo,agua,terra,vento,raio,nature,dano_sin)
